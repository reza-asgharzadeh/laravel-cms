            ...
            <div class="comments__header">
                ...
                <!-- Reply form start -->
                    <div class="d-flex flex-column">
                        ...
                        @endauth
                        <form action="{{route('reply.comment.episode.store',[$episode_id,$comment->id])}}" method="post" class="reply-form d-none" id="comment{{$comment->id}}">
                            @csrf
        ...
        @if($comment->approvedReplies->count() > 0)
            <div class="comments__subset">
                @foreach($comment->approvedReplies as $reply)
                    @include('client.landing_comments.episode', ['comment' => $reply])
                @endforeach
            </div>
        @endif
    </div>
